MyRecipeBoxes.controller("Recipes",['FIREBASE_URL','$scope','$location','$firebase','firebaseRefManager','$routeParams','$rootScope',function(FIREBASE_URL,$scope,$location,$firebase,firebaseRefManager,$routeParams,$rootScope)
{
	$scope.box = $firebase(firebaseRefManager(FIREBASE_URL+"boxes").child($routeParams.box_id));
	$scope.boxes = $firebase(firebaseRefManager(FIREBASE_URL+"boxes").startAt($rootScope.user.uid).endAt($rootScope.user.uid));
	...
	$scope.save_recipe = function()
	{
		...
		if($routeParams.box_id !== $scope.box_$id)
		{
			...
			$scope.box = $firebase(firebaseRefManager(FIREBASE_URL+"boxes").child($scope.box_$id));
